            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              ...
              <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:6}}>
                ...
                <div style={{display:"flex",gap:6}}>
                  ...
                  <datalist id="categorias-modelos">{[...new Set(modelos.map(m=>m.categoria||"Geral"))].map(c=><option key={c} value={c}/>)}</datalist>
                <div onClick={async()=>{
...
function Prontuario({p1, equipeGlobal}) {
  ...
  const [editandoKey, setEditandoKey] = React.useState(null);
  ...
  const [pagModoAss, setPagModoAss] = React.useState("presencial");
  ...
  const iniciarEdicao = (e) => {
    setEditandoKey(e._key);
    setNovaData(e.data||new Date().toISOString().split("T")[0]);
    setNovaDescricao(e.descricao||"");
    setNovaObsInterna(e.obsInterna||"");
    setNovoDentistaResp(e.dentistaResponsavel||"");
  ...
  const excluirEntrada = (key) => {
    if(!window.confirm("Excluir esta entrada do prontuário? Esta ação não pode ser desfeita.")) return;
    if(!_fbDb) { showToast("Sem conexão com a nuvem no momento.","error"); return; }
    _fbDb.ref(PRONTUARIO_FB_PATH+"/"+key).remove()
  ...
  const salvarEntrada = () => {
    ...
    if(incluirPagamento) {
      if(!pagValorNum || pagValorNum<=0) { showToast("Informe o valor recebido do pagamento.","error"); return; }
    ...
    const criarPagamentoVinculado = () => {
      ...
      const camposPag = {
        ...
        tipo:"pagamento", valor: pagValorNum, forma: pagForma, destinatario: pagDestinatario,
        obsInterna: pagObs.trim(), dentistaResponsavel: novoDentistaResp,
        assinaturaDentista: assDentista, assinaturaPaciente: assPaciente,
    ...
    const aoTerminar = () => {
      ...
      showToast(editandoKey ? (incluirPagamento ? "Entrada atualizada e pagamento registrado." : "Entrada atualizada.") : (incluirPagamento && modoAssPaciente==="presencial" ? "Entrada e pagamento registrados no prontuário" : incluirPagamento && modoAssPaciente==="link" ? "Entrada salva — o link já inclui o pagamento a confirmar" : modoAssPaciente==="presencial" ? "Entrada registrada no prontuário" : "Entrada salva — copie o link para o paciente assinar"));
    };
    const aoFalhar = e => { setSalvando(false); showToast("Erro ao salvar: "+e.message,"error"); };
    if(editandoKey) {
      _fbDb.ref(PRONTUARIO_FB_PATH+"/"+editandoKey).update({...campos, editadoEm:new Date().toISOString()}).then(criarPagamentoVinculado).then(aoTerminar).catch(aoFalhar);
    } else {
      const key = _fbDb.ref(PRONTUARIO_FB_PATH).push().key;
      _fbDb.ref(PRONTUARIO_FB_PATH+"/"+key).set({...campos, criadoEm:new Date().toISOString()}).then(criarPagamentoVinculado).then(aoTerminar).catch(aoFalhar);
    }
  };
  ...
  const iniciarEdicaoFalta = (e) => {
    setEditandoFaltaKey(e._key);
  ...
  const registrarFalta = () => {
    ...
    const onus = faltaAntecedencia==="menos24";
    const descricao = (faltaOcorrencia==="remarcou" ? "Paciente remarcou a consulta." : "Paciente não compareceu à consulta.")
    ...
    const aoTerminar = () => {
      setFaltaSalvando(false); setShowFalta(false); resetFalta();
      showToast(editandoFaltaKey ? "Falta atualizada." : "Falta registrada no prontuário.");
    };
    const aoFalhar = e => { setFaltaSalvando(false); showToast("Erro ao salvar: "+e.message,"error"); };
    if(editandoFaltaKey) {
      _fbDb.ref(PRONTUARIO_FB_PATH+"/"+editandoFaltaKey).update({...campos, editadoEm:new Date().toISOString()}).then(aoTerminar).catch(aoFalhar);
    } else {
      const key = _fbDb.ref(PRONTUARIO_FB_PATH).push().key;
  ...
  const resetPagamento = () => {
    ...
    setPagDentista(""); setPagAssDentista(""); setPagAssPaciente(""); setPagModoAss("presencial");
  };
  ...
  const iniciarEdicaoPagamento = (e) => {
    ...
    setPagModoAss(e.assinaturaPaciente?"presencial":"link");
  ...
  const registrarPagamento = () => {
    ...
    const campos = {
      ...
      assinaturaDentista: pagAssDentista, assinaturaPaciente: pagModoAss==="presencial" ? pagAssPaciente : "",
      statusPaciente: pagModoAss==="presencial" ? "assinado" : "pendente",
    };
    const aoTerminar = () => {
      setPagSalvando(false); setShowPagamento(false); resetPagamento();
      showToast(editandoPagamentoKey ? "Pagamento atualizado." : (pagModoAss==="presencial" ? "Pagamento registrado no prontuário." : "Pagamento salvo — copie o link para o paciente assinar."));
    };
  ...
  const carregarScans = () => {
    ...
    onFirebaseReady(async ()=>{
      try {
        ...
        const lista = await gdriveListarScansComPreview(cpfPaciente);
        setScans(lista);
      } catch(e) { setScans([]); }
    });
  };
  ...
  return (
    <div style={{padding:"16px 16px 90px",maxWidth:640,margin:"0 auto"}}>
        ...
        <div className="no-print" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",padding:"20px 12px"}}>
          <div style={{background:"#fff",borderRadius:8,padding:20,maxWidth:420,width:"100%",marginTop:20,marginBottom:20}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:6,marginBottom:16}}>
              <div onClick={()=>setFaltaOcorrencia("faltou")} style={{flex:1,padding:"9px 8px",borderRadius:4,fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"center",border:"1.5px solid #C62828",background:faltaOcorrencia==="faltou"?"#C62828":"#fff",color:faltaOcorrencia==="faltou"?"#fff":"#C62828"}}>Faltou</div>
              <div onClick={()=>setFaltaOcorrencia("remarcou")} style={{flex:1,padding:"9px 8px",borderRadius:4,fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"center",border:"1.5px solid #C62828",background:faltaOcorrencia==="remarcou"?"#C62828":"#fff",color:faltaOcorrencia==="remarcou"?"#fff":"#C62828"}}>Remarcou</div>
            </div>

            <label style={{fontSize:10,fontWeight:700,color:GOLD_DARK,textTransform:"uppercase"}}>Consulta estava confirmada previamente?</label>
            <div style={{display:"flex",gap:8,marginTop:6,marginBottom:16}}>
              <div onClick={()=>setFaltaConfirmou(true)} style={{flex:1,padding:"9px 8px",borderRadius:4,fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"center",border:"1.5px solid "+PURPLE,background:faltaConfirmou===true?PURPLE:"#fff",color:faltaConfirmou===true?"#fff":PURPLE}}>Sim</div>
              <div onClick={()=>setFaltaConfirmou(false)} style={{flex:1,padding:"9px 8px",borderRadius:4,fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"center",border:"1.5px solid "+PURPLE,background:faltaConfirmou===false?PURPLE:"#fff",color:faltaConfirmou===false?"#fff":PURPLE}}>Não</div>
            </div>

            <label style={{fontSize:10,fontWeight:700,color:GOLD_DARK,textTransform:"uppercase"}}>Antecedência do aviso</label>
            <div style={{display:"flex",gap:8,marginTop:6,marginBottom:8}}>
              <div onClick={()=>setFaltaAntecedencia("mais24")} style={{flex:1,padding:"9px 8px",borderRadius:4,fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"center",border:"1.5px solid "+GOLD_DARK,background:faltaAntecedencia==="mais24"?GOLD_DARK:"#fff",color:faltaAntecedencia==="mais24"?"#fff":GOLD_DARK}}>Mais de 24h</div>
              <div onClick={()=>setFaltaAntecedencia("menos24")} style={{flex:1,padding:"9px 8px",borderRadius:4,fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"center",border:"1.5px solid "+GOLD_DARK,background:faltaAntecedencia==="menos24"?GOLD_DARK:"#fff",color:faltaAntecedencia==="menos24"?"#fff":GOLD_DARK}}>Menos de 24h / sem aviso</div>
            </div>
            {faltaAntecedencia && (
              <div style={{fontSize:11,fontWeight:600,marginBottom:16,color:faltaAntecedencia==="menos24"?"#C62828":"#2E7D32"}}>
                {faltaAntecedencia==="menos24" ? "→ Sujeito a ônus para paciente de convênio." : "→ Sem ônus para paciente de convênio."}
              </div>
            )}
      ...
      {showPagamento && (
        <div className="no-print" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",padding:"20px 12px"}}>
          <div style={{background:"#fff",borderRadius:8,padding:20,maxWidth:420,width:"100%",marginTop:20,marginBottom:20}}>
            ...
            <div style={{fontSize:10,color:"#9A8060",marginTop:4,marginBottom:6}}>Confirma o pagamento — sempre necessária, mesmo que o procedimento do dia já tenha sido assinado.</div>
            <div style={{display:"flex",gap:8,marginTop:6,marginBottom:12}}>
              <div onClick={()=>setPagModoAss("presencial")} style={{flex:1,textAlign:"center",padding:"10px 7px",borderRadius:4,fontSize:12,fontWeight:700,cursor:"pointer",background:pagModoAss==="presencial"?"#2E7D32":"#F4FAF5",color:pagModoAss==="presencial"?"#fff":"#2E7D32"}}>Assinar agora</div>
              <div onClick={()=>setPagModoAss("link")} style={{flex:1,textAlign:"center",padding:"10px 7px",borderRadius:4,fontSize:12,fontWeight:700,cursor:"pointer",background:pagModoAss==="link"?"#2E7D32":"#F4FAF5",color:pagModoAss==="link"?"#fff":"#2E7D32"}}>Enviar link depois</div>
            </div>
            {pagModoAss==="presencial" ? (
              <div style={{marginBottom:16}}><AssinaturaCanvas value={pagAssPaciente} onChange={setPagAssPaciente}/></div>
            ) : (
              <div style={{fontSize:12,color:"#9A8060",marginBottom:16,lineHeight:1.5}}>Depois de salvar, copie o link e mande pelo WhatsApp pro paciente assinar remotamente. Ele verá o valor e a forma de pagamento — nada além disso.</div>
            )}
      ...
      {showNova && (
        <div className="no-print" style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",padding:"20px 12px"}}>
          <div style={{background:"#fff",borderRadius:8,padding:20,maxWidth:480,width:"100%",marginTop:20,marginBottom:20}}>
            <div style={{fontSize:15,fontWeight:700,color:"#2A1538",marginBottom:14}}>{editandoKey?"Editar entrada":"Nova entrada"} — {p1.nome||"paciente sem nome"}</div>
              ...
              <div style={{marginTop:4,marginBottom:16,paddingTop:14,borderTop:"1px dashed "+BORDER}}>
                <div onClick={()=>setIncluirPagamento(!incluirPagamento)} style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",padding:"6px 4px"}}>
                  <div style={{width:20,height:20,borderRadius:4,border:"2px solid "+(incluirPagamento?"#2E7D32":BORDER),background:incluirPagamento?"#2E7D32":"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    {incluirPagamento&&<span style={{color:"#fff",fontSize:13,fontWeight:900}}>✓</span>}
                  </div>
                  <div style={{fontSize:13,fontWeight:700,color:incluirPagamento?"#2E7D32":"#2A1538"}}>💰 Também registrar pagamento nesta visita</div>
                </div>

                {incluirPagamento && (
                  <div style={{marginTop:10,padding:"12px 14px",background:"#F4FAF5",border:"1px solid #2E7D32",borderRadius:6}}>
                    <label style={{fontSize:10,fontWeight:700,color:"#2E7D32",textTransform:"uppercase"}}>Valor recebido</label>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginTop:4,marginBottom:14}}>
                      <span style={{fontSize:16,color:"#5C4A2A",fontWeight:700}}>R$</span>
                      <input value={pagValor} onChange={e=>setPagValor(e.target.value)} inputMode="decimal" placeholder="0,00" style={{flex:1,padding:"11px 10px",border:"1px solid "+BORDER,borderRadius:4,fontSize:16,fontFamily:"inherit",boxSizing:"border-box",background:"#fff"}}/>
                    </div>

                    <label style={{fontSize:10,fontWeight:700,color:"#2E7D32",textTransform:"uppercase"}}>Forma de pagamento</label>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:6,marginBottom:14}}>
                      {[["dinheiro","Dinheiro"],["pix","PIX"],["debito","Débito"],["credito","Crédito"],["boleto","Boleto"]].map(([k,l])=>(
                        <div key={k} onClick={()=>setPagForma(k)} style={{padding:"8px 11px",borderRadius:20,fontSize:11,fontWeight:700,cursor:"pointer",border:"1.5px solid #2E7D32",background:pagForma===k?"#2E7D32":"#fff",color:pagForma===k?"#fff":"#2E7D32"}}>{l}</div>
                      ))}
                    </div>

                    <label style={{fontSize:10,fontWeight:700,color:"#2E7D32",textTransform:"uppercase"}}>Valor foi para</label>
                    <div style={{display:"flex",gap:8,marginTop:6,marginBottom:6}}>
                      <div onClick={()=>setPagDestinatario("dentista")} style={{flex:1,padding:"9px 8px",borderRadius:4,fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"center",border:"1.5px solid "+PURPLE,background:pagDestinatario==="dentista"?PURPLE:"#fff",color:pagDestinatario==="dentista"?"#fff":PURPLE}}>Dentista</div>
                      <div onClick={()=>setPagDestinatario("clinica")} style={{flex:1,padding:"9px 8px",borderRadius:4,fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"center",border:"1.5px solid "+PURPLE,background:pagDestinatario==="clinica"?PURPLE:"#fff",color:pagDestinatario==="clinica"?"#fff":PURPLE}}>Clínica</div>
                    </div>
                    <div style={{fontSize:9,color:"#9A8060",marginBottom:12}}>Usado depois para acertar a compensação de 50%/50% entre as partes.</div>
